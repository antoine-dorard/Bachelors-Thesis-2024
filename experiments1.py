...
def gpt_setting_1(openai_client, method, system_message, model="gpt-3.5-turbo", temp=1):
    message = f"""
...
def gpt_setting_2(openai_client, method, system_message, model="gpt-3.5-turbo", temp=1):
...
def gpt_setting_3(openai_client, method, system_message, model="gpt-3.5-turbo", temp=1):
    message = f"""
                -- Information provided --
...
def gpt_setting_4(openai_client, method, system_message, model="gpt-3.5-turbo", temp=1):
...
"""
    ...
    if setting == 1:
        return gpt_setting_1(openai_client, method, system_message, model, temp)
    elif setting == 2:
        return gpt_setting_2(openai_client, method, system_message, model, temp)
    elif setting == 3:
        return gpt_setting_3(openai_client, method, system_message, model, temp)
    elif setting == 4:
        return gpt_setting_4(openai_client, method, system_message, model, temp)
    
    raise ValueError("setting must be between 1 and 4")


def evaluate_app(app, mobsf_results, file_objects, vulnerable_methods, setting_nb, explain=False):
    y = []
    y_true = []
    
    app_name = os.path.basename(os.path.normpath(app))
    
    f = open(f"experiments/results_exp1/{app_name}_all_results_{setting_nb}.csv", "a")
            ...
            for vulnerable_file in vulnerability["files"]:
                if vulnerable_file.get("label") is not None:
                    ...
                    if vulnerable_file.get("method_hash") is not None:
                        ...
                        gpt_result = ask_gpt(openai_client, method, setting_nb, temp=0.7, explain=explain)
...
def evaluate(args):
    ...
    for app in apps:
        ...
        for sett in range(1, 5): # setting 1 to 4
                
            print(f"== Evaluating {app} for {sett} ==")
            for i in range(15):
                print("Iteration", i, "setting", sett)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("experiments/results_exp1"):
        os.makedirs("experiments/results_exp1")
